...
import { Bolt, Copy, Download, Trash2 } from 'lucide-react';
...
export function ImportApps() {
  ...
  return (
    <div>
      <div className="mb-5"><h1 className="page-title">Import Your Apps</h1><p className="page-sub">Device-specific import — iPhone, iPad & Mac each handled differently</p></div>

      <div className="flex gap-2 mb-4">
        {(['iphone', 'ipad', 'mac'] as const).map((d) => (
          <button key={d} type="button" onClick={() => setDevice(d)} className={`device-tab ${device === d ? 'active' : ''}`}>{d === 'iphone' ? '📱 iPhone' : d === 'ipad' ? '📱 iPad' : ' Mac'}</button>
        ))}
      </div>
      ...
      <GlassCard className="mb-3 border-[rgba(79,110,247,0.28)]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1"><p className="font-bold text-sm">One-Click Import Shortcut</p><p className="text-[11px] text-[var(--mu)]">Copies ALL installed app names to clipboard in one tap</p></div>
      ...
      >
        ...
        {device === 'mac' && (
          <div className="mt-3 p-2.5 rounded-lg bg-[var(--s1)] border border-[var(--bd2)]">
            <code className="text-[11px] text-[var(--ac3)] font-mono">ls /Applications | sed 's/.app//'</code>
            <button type="button" onClick={() => { navigator.clipboard?.writeText("ls /Applications | sed 's/.app//'"); toast('Command copied!'); }} className="btn-ghost btn-sm ml-2"><Copy size={12} /></button>
          </div>
        )}
      </GlassCard>
        ...
        <GlassCard className="mb-3">
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[{ v: result.added, l: 'Added', c: 'var(--ac3)' }, { v: result.existed, l: 'Already in', c: 'var(--mu)' }, { v: state.apps.length, l: 'Total', c: 'var(--ac)' }].map((x) => (
              <div key={x.l} className="stat-row text-center flex-col !items-center"><div className="text-2xl font-extrabold font-display" style={{ color: x.c }}>{x.v}</div><div className="text-[11px] text-[var(--mu)]">{x.l}</div></div>
            ))}
          </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="One-Click Import Shortcut">
        ...
        <button type="button" onClick={() => { setModal(false); toast('Go to the paste box below!'); }} className="btn-primary w-full">Go to Import Tab</button>
      </Modal>
