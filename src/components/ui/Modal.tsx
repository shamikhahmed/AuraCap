...
export function Modal({ open, onClose, title, wide, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[400] flex items-center justify-center p-5 bg-black/72 backdrop-blur-sm" onClick={onClose}>
      <div className={`bg-[var(--s2)] border border-[var(--bd2)] rounded-[22px] p-6 w-full max-h-[90vh] overflow-y-auto relative ${wide ? 'max-w-2xl' : 'max-w-md'}`} onClick={(e) => e.stopPropagation()}>
